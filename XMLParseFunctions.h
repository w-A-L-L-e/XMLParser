...
int XMLParser::yyerror( string s ) {
  cerr<<"SYNTAX ERROR at line "   <<getXMLLineNr()<<", col "<<getXMLColumn()<<" : "<< s <<endl;
  //exit(-1);
  bSynError=1;
  YYACCEPT;
  return 0;
...
int XMLParser::semanticError( string s ){
  cerr<<"SEMANTIC ERROR at line " <<getXMLLineNr()<<", col "<<getXMLColumn()<<" : "<< s <<endl;
  bSemError=1;
  return 0;
}

void XMLParser::warning( string s ) {
